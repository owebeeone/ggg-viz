...
export const S_SYNC: Scenario = {
  ...
  steps: [
    ...
    },
    {
      ...
      payload: { share: 'ws-razel', detail: { mine: 'empty vector', theirs: 'per-(origin, zone) (seq, chain-head) — the chain unit after D8', bonus: 'same seq + DIFFERENT head here would already be fork evidence (per chain)' } },
      note: 'Version vectors compute the exact gap both ways — and double as the first tamper tripwire. Chains are per-(origin, zone) (D8): a peer can withhold an ENTIRE private-zone chain and every other chain still verifies — privacy filtering never breaks integrity. local2 receives only the zones its grants admit; absent chains are absences, not holes.',
      docRef: `${SUB_V1} §2 (GQ-9) · ${AZ} §4a (D8: per-(origin, zone) chains)`,
    },
    {
      state: 'Y1', phase: 'SY1', kind: 'internal', from: 'local2', frame: 'FOLD',
      label: 'checkpoint accepted (anchored)',
      payload: { share: 'ws-razel', detail: { checkpoint: 'chain (peer1·commons), seq 12,000, signed by origin', anchor: 'checkpoint head ALSO in the home-share fold — rewrite would diverge the folds' } },
    ...
    },
    {
      ...
      payload: { share: 'ws-razel', detail: { range: '(peer1·commons, 12,001..14,102)', priority: 'BULK — size-capped chunks; never blocks interactive traffic' } },
    ...
    },
    {
      ...
      payload: { share: 'ws-razel', detail: { range: '(peer1·commons, 14,103..14,480)' } },
    ...
    },
    {
      ...
      payload: { share: 'ws-razel', detail: { range: '(peer1·commons, 14,481..14,499)', problem: 'op 14,490 modified in transit' } },
    ...
    },
    {
      ...
      payload: { detail: { failure: 'prev-hash mismatch at (peer1·commons, 14,490)', action: 'reject op + suffix FROM THIS PEER for this (origin, zone) chain; re-fetch elsewhere' } },
      ...
      note: 'local1 is marked suspect for this chain; nothing it sent after the break is kept.',
      docRef: `${SUB_V1} §2 · ${AZ} §7a`,
      sets: { local2: { 'suspect local1': 'ws-razel/peer1·commons suffix rejected @14,490' } },
    },
    {
      state: 'B8', phase: 'SY3', kind: 'message', from: 'peer1', to: 'local2', frame: 'OPS',
      label: 're-fetch from another replica', response: true,
      payload: { share: 'ws-razel', detail: { range: '(peer1·commons, 14,481..14,499) — resume is exact', via: 'iroh dial compressed' } },
    ...
    },
    {
      ...
      label: 'op A for slot (peer1·commons, 14,500)', response: true,
      payload: { share: 'ws-razel', detail: { op: '(peer1·commons, 14,500) = A — SIGNED by peer1' } },
    ...
    },
    {
      ...
      payload: { share: 'ws-razel', detail: { op: '(peer1·commons, 14,500) = B — also SIGNED by peer1' } },
    ...
    },
    {
      ...
      payload: { detail: { evidence: 'two signed ops, one (origin, zone, seq) slot — a self-contained fork proof', action: 'origin flagged; share surfaced to UI; proof replicates like any record' } },
      ...
      sets: { local2: { evidence: 'peer1 FORKED at ws-razel/peer1·commons@14,500 (proof held)' } },
    },
  ],
};
